...
class Character:
    ...
    def acquire_skill(self, skill: Skill, upgrade=False) -> bool:
        acquired: bool = False
    ...
    def qualify(self, ct: CareerType, drafted=False) -> (bool, Optional[List[str]]):
        ...
        if drafted:
            ...
            v = Random().randint(1, 6) + Random().randint(1, 6) + self.modifier(ct.qualification[0]) - 2 * len(self.careers)
    ...
    def survival_roll(self) -> bool:
        ...
        v = Random().randint(1, 6) + Random().randint(1, 6) + self.modifier(c.career_type.survival[0])
        return v >= c.career_type.survival[1]

    def mishaps_roll(self) -> str:
        physical_char: List[Characteristic] = [Characteristic.END, Characteristic.STR, Characteristic.DEX]
        char: Characteristic
        message: str

        mishaps = Random().randint(1, 6)
        if mishaps == 1:
            char = Random().choice(physical_char)
            ...
            if injury == 1:
                ...
                char = physical_char[Random().randint(1, 2)]
                self.stats[char] -= 2
        ...
        for c in Characteristic:
            ...
            if self.stats[c] == 0:
                self.injury_crisis_debt += 10_000 * Random().randint(1, 6)
                self.stats[c] = 1
    ...
    def commission_roll(self) -> Optional[bool]:
        if self.career.rank == 0 and self.career.career_type.commission:
            v = Random().randint(1, 6) + Random().randint(1, 6) + self.modifier(self.career.career_type.commission[0])
    ...
    def advancement_roll(self) -> Optional[bool]:
        if self.career.rank >= 1 and self.career.career_type.advancement:
            v = Random().randint(1, 6) + Random().randint(1, 6) + self.modifier(self.career.career_type.advancement[0])
    ...
    def skills_and_training(self, table_name: str) -> Union[Characteristic, str]:
        index = ['Personal Development', 'Service Skills', 'Specialist Skills', 'Advanced Education'].index(table_name)

        upgrade: Union[Characteristic, str] = Random().choice(list(self.career.career_type.skill_and_training[index].values()))
        if isinstance(upgrade, Characteristic):
            self.stats[upgrade] += 1
        else:
            self.acquire_skill(Skill(upgrade, 0), upgrade=True)

        return upgrade

    def use_drugs(self, use: bool):
        if use:
            self.drugs_terms += 1
            self.debt += 2500
    ...
    def aging_roll(self):  # TODO Return something
    ...
    def roll_benefit(self, cash: bool):
        self.benefit_rolls -= 1
        if cash:
            v = dice.roll()
            ...
            if isinstance(benefit, Characteristic):
                ...
                if benefit == 'Society':
                    ...
                    for eq, qt in self.inventory:
                        if eq.id == weapon_id:
                            found = True
                            if benefit in ['LightWeapon', 'HeavyWeapon', 'Sword']:
                                self.acquire_skill(Skill('Melee Combat', 0), upgrade=True)
                            else:
                                self.acquire_skill(Skill('Gun Combat', 0), upgrade=True)
                            break
    ...
    @property
    def available_careers(self) -> List[str]:
        cs = list(careers.keys())
        for c in self.careers:
            if c.career_type.name in cs and c.career_type.name != 'Drifter':
                cs.remove(c.career_type.name)
